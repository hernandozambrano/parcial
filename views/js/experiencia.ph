$(document).ready(function () {
    $('#tablaExperiencia').DataTable({
        "ajax": {
            "url": "/PaginaHZ/controller/experiencia.php?op=listar",
            "type": "GET",
            "dataSrc": ""
        },
        "columns": [
...
function guardarExperiencia() {
    ...
    $.post(`/PaginaHZ/controller/experiencia.php?op=${operacion}`, { id, empresa, puesto, fecha_inicio, fecha_fin, descripcion }, function (response) {
        if (response === 'success') {
...
function cargarDatosModal(id) {
    $.post('/PaginaHZ/controller/experiencia.php?op=mostrar', { id }, function (response) {
...
function eliminarExperiencia(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.post('/PaginaHZ/controller/experiencia.php?op=eliminar', { id }, function (response) {
                if (response === 'success') {
